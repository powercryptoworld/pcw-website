...
// Portal root to render picker at page top (prevents clipping/scroll jump)
...
// === Swap state (EVM) ===
  ...
  const [pickerTop, setPickerTop] = useState<number>(12);

  // Keep picker at top & prevent background scroll while open
  useEffect(() => {
    if (picker) {
        ...
        <div ref={cardRef} className="glass swapCard relative" style={{ overflow: picker ? "visible" : undefined }}>
            ...
            {picker && (
              <>
                ...
                <div
                  className="absolute inset-0 z-[9998] bg-black/40 rounded-2xl"
...
<div className="mt-3 space-y-2 max-h-72 overflow-auto pr-1">
            ...
            {/* picker (now inside card, pinned to top) */}
          ...
          {/* You pay */}
          <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2"><button className="token-chip-btn chip-icon-only" onClick={() => { try { window.scrollTo(0,0) } catch {} ; setPicker("pay"); }} title="Choose Pay token">{solPay ? (
            ...
            )}</button><button onClick={() => { try { window.scrollTo(0,0) } catch {} ; setPicker("pay"); }} className="text-xs px-2 py-1 rounded bg-white/10 hover:bg-white/20">Pay token</button></div></div>
          <TokenRow
            title=""
          ...
          {/* You receive */}
          <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2"><button className="token-chip-btn chip-icon-only" onClick={() => { try { window.scrollTo(0,0) } catch {} ; setPicker("receive"); }} title="Choose Receive token">{solReceive ? (
            ...
            )}</button><button onClick={() => { try { window.scrollTo(0,0) } catch {} ; setPicker("receive"); }} className="text-xs px-2 py-1 rounded bg-white/10 hover:bg-white/20">Receive token</button></div></div>
          <TokenRow
            title=""
