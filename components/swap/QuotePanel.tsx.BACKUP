...
export default function QuotePanel({ chainId, src, dst, amount, defaultSlippageBps = 50 }: Props) {
  ...
  const nativeUsd = useUsdQuote(chainId, nativeAddr, 18).priceUsd;
    ...
    const hasHumanAmount = useMemo(() => !!amount && Number(amount) > 0, [amount]);
// Gas tiers from our lab helper
  ...
  const gasUsd = useMemo(() => {
    if (!hasHumanAmount || !gasUnits || !selectedGasWei || !nativeUsd || nativeUsd <= 0) return undefined;
  ...
  }, [chainId, nativeAddr, nativeUsd, gasUnits, selectedGasWei, gasGwei, gasUsd, quoteGasStr, speed]);
  ...
  useEffect(() => {
    ...
    async function handleSwapClick() {
      try {
        ...
        // Try to switch to the right chain (ok if this fails)
        try {
          const chainHex = "0x" + Number(chainId).toString(16);
          await meta.request({
            method: "wallet_switchEthereumChain",
          ...
          console.warn("chain switch skipped/failed", err);
        }
...
//     const canSwap =
    ...
    async function handleSwapClick() {
      try {
        ...
        if (!j?.ok || !j?.tx) {
          console.error("Swap tx error", j);
          alert("Swap quote not available.");
          return;
        }
  ...
  return (
    <div className="mt-3 rounded-2xl bg-black/25 border border-white/10 p-3 text-sm">
      ...
      <div className="mt-2">
        ...
        <div className="text-base">
          {dstAmtStr ? (gasUsd != null ? formatUsd(gasUsd) : "—") : "—"}
        </div>
        <div className="hidden mt-1 text-[11px] opacity-60">
      ...
      {/* Execution (preview) */}
      <div className="mt-4 border-t border-white/10 pt-3 hidden">
